...
export default function ProductDetailPage() {
  ...
  const [product, setProduct] = useState<ProductWithMongoId | null>(null);
  ...
  const [relatedProducts, setRelatedProducts] = useState<ProductWithMongoId[]>([]);

  useEffect(() => {
    if (product?.category) {
      api.get(`/products?category=${product.category}&limit=5`)
        .then(res => {
          const fetched = (res.data.data || []) as ProductWithMongoId[];
          const currentId = product.id || product._id;
          setRelatedProducts(fetched.filter((p) => (p.id || p._id) !== currentId).slice(0, 4));
        })
        .catch(console.error);
    }
  }, [product]);
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <ProductCard key={rp.id || rp._id} product={rp} viewMode="grid" />
              ))}
            </div>
          </div>
        )}
