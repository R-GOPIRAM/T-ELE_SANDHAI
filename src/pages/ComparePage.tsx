...
export default function ComparePage() {
    ...
    const attributes: Array<{ label: string; key: keyof Product; icon: ComponentType<{ className?: string }>; format?: (val: unknown) => string; specPath?: string }> = [
    ...
    const getAttributeValue = (product: Product, attr: (typeof attributes)[number]) => {
        if (attr.specPath) {
            return product.specifications?.[attr.specPath] || 'N/A';
        }
        const val = product[attr.key] as unknown;
        if (attr.format) return attr.format(val);
